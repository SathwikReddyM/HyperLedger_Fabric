...
let AssetTransferContract = class AssetTransferContract extends fabric_contract_api_1.Contract {
    async InitLedger(ctx) {
        ...
        for (const asset of assets) {
            ...
            await ctx.stub.putState(asset.Case_ID, Buffer.from((0, json_stringify_deterministic_1.default)((0, sort_keys_recursive_1.default)(asset))));
            console.info(`Asset ${asset.Case_ID} initialized`);
        }
    }
    // CreateAsset issues a new asset to the world state with given details.
    async CreateAsset(ctx, Case_ID, Evidence_ID, State_ID, Removal_Owner, Removal_reason) {
        ...
        // we insert data in alphabetic order using 'json-stringify-deterministic' and 'sort-keys-recursive'
        await ctx.stub.putState(Case_ID, Buffer.from((0, json_stringify_deterministic_1.default)((0, sort_keys_recursive_1.default)(asset))));
    }
    // ReadAsset returns the asset stored in the world state with given id.
    async ReadAsset(ctx, Case_ID) {
    ...
    async UpdateAsset(ctx, Case_ID, Evidence_ID, State_ID, Removal_Owner, Removal_reason) {
        const exists = await this.AssetExists(ctx, Case_ID);
        if (!exists) {
            throw new Error(`The asset ${Case_ID} does not exist`);
        }
        ...
        // we insert data in alphabetic order using 'json-stringify-deterministic' and 'sort-keys-recursive'
        return ctx.stub.putState(Case_ID, Buffer.from((0, json_stringify_deterministic_1.default)((0, sort_keys_recursive_1.default)(updatedAsset))));
    }
    // DeleteAsset deletes an given asset from the world state.
    async DeleteAsset(ctx, Case_ID) {
        const exists = await this.AssetExists(ctx, id);
        if (!exists) {
            throw new Error(`The asset ${Case_ID} does not exist`);
        }
        return ctx.stub.deleteState(Case_ID);
    }
    // AssetExists returns true when asset with given ID exists in world state.
    async AssetExists(ctx, Case_ID) {
        const assetJSON = await ctx.stub.getState(Case_ID);
        return assetJSON && assetJSON.length > 0;
    }
    // TransferAsset updates the owner field of asset with given id in the world state, and returns the old owner.
    async TransferAsset(ctx, Case_ID, State_ID) {
    ...
    /*async RemoveAsset(ctx, Case_ID, State_ID) {
        const assetString = await this.ReadAsset(ctx, Case_ID);
        const asset = JSON.parse(assetString);
        const oldOwner = asset.State_ID;
        asset.State_ID = State_ID;
